...
func (r *InventoryRepository) createFilterSQLForReturnMerchandiseAuthorizationViews(colID string, filterItem query.FilterItem) string {
	...
	case "date":
		return r.createDateFilterSQLForReturnMerchandiseAuthorizationViews(colID, filterItem)
	default:
		return "true"
	}
}
	...
	if filterItem.Operator != "" {
		...
		case "AND":
			...
			for _, condition := range conditions {
				andParts = append(andParts, r.createNumberFilterSQLForReturnMerchandiseAuthorizationViews(colID, condition))
			}
			return strings.Join(andParts, " AND ")
		case "OR":
			var orParts []string
			for _, condition := range conditions {
				orParts = append(orParts, r.createNumberFilterSQLForReturnMerchandiseAuthorizationViews(colID, condition))
			}
...
func (r *InventoryRepository) createTextFilterSQLForReturnMerchandiseAuthorizationViews(colID string, filterModel query.FilterItem) string {
	...
	if filterModel.Operator != "" {
		...
		case "AND":
			var andParts []string
			for _, condition := range conditions {
				andParts = append(andParts, r.createTextFilterSQLForReturnMerchandiseAuthorizationViews(colID, condition))
			}
			return strings.Join(andParts, " AND ")
		case "OR":
			var orParts []string
			for _, condition := range conditions {
				orParts = append(orParts, r.createTextFilterSQLForReturnMerchandiseAuthorizationViews(colID, condition))
			}
			return strings.Join(orParts, " OR ")
		default:
			return "false"
...
func (r *InventoryRepository) createDateFilterSQLForReturnMerchandiseAuthorizationViews(colID string, filterModel query.FilterItem) string {
	// Handle filter operator and conditions (recursive)
	if filterModel.Operator != "" {
		conditions := filterModel.Conditions
		switch filterModel.Operator {
		case "AND":
			var andParts []string
			for _, condition := range conditions {
				andParts = append(andParts, r.createDateFilterSQLForReturnMerchandiseAuthorizationViews(colID, condition))
			}
			return strings.Join(andParts, " AND ")
		case "OR":
			var orParts []string
			for _, condition := range conditions {
				orParts = append(orParts, r.createDateFilterSQLForReturnMerchandiseAuthorizationViews(colID, condition))
			}
			return strings.Join(orParts, " OR ")
		default:
			return "false"
