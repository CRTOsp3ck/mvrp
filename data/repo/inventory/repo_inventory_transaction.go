...
func (r *InventoryRepository) SearchInventoryTransactions(ctx context.Context, exec boil.ContextExecutor, dto dto.SearchInventoryTransactionDTO) (inventory.InventoryTransactionSlice, int, error) {
...
func (r *InventoryRepository) BuildSearchQueryForInventoryTransactions(ctx context.Context, exec boil.ContextExecutor, dto dto.SearchInventoryTransactionDTO) (inventory.InventoryTransactionSlice, int, error) {
	var queryMods []qm.QueryMod
	if dto.InventoryId != "" {
		queryMods = append(queryMods, qm.Where("inventory_id = ?", dto.InventoryId))
	}
	...
	selectSQL := r.createSelectSQLForInventoryTransactions(request)
	if selectSQL != "" {
		queryMods = append(queryMods, qm.Select(selectSQL))
	}

	whereSQL := r.createWhereSQLForInventoryTransactions(request)
	if whereSQL != "" {
		queryMods = append(queryMods, qm.Where(whereSQL))
	}

	groupBySQL := r.createGroupBySQLForInventoryTransactions(request)
	if groupBySQL != "" {
		queryMods = append(queryMods, qm.GroupBy(groupBySQL))
	}

	orderBySQL := r.createOrderBySQLForInventoryTransactions(request)
	if orderBySQL != "" {
		queryMods = append(queryMods, qm.OrderBy(orderBySQL))
	}

	limitSQL, offsetSQL := r.createLimitAndOffsetSQLForInventoryTransactions(request)
	if limitSQL > 0 {
...
// 1. Create Select SQL
func (r *InventoryRepository) createSelectSQLForInventoryTransactions(request query.IServerSideGetRowsRequest) string {
    ...
    if len(rowGroupCols) > len(groupKeys) {
        ...
        for i := 0; i <= len(groupKeys) && i < len(rowGroupCols); i++ {
            colsToSelect = append(colsToSelect, r.handleNestedFieldsForInventoryTransactions(rowGroupCols[i].Field))
        }

        // Include aggregated columns in SELECT
        for _, valueCol := range valueCols {
            colsToSelect = append(colsToSelect, fmt.Sprintf("SUM(%s) AS %s", r.handleNestedFieldsForInventoryTransactions(valueCol.Field), valueCol.Field))
        }
    } else {
        // If not grouping, select all columns directly
        for _, valueCol := range valueCols {
            colsToSelect = append(colsToSelect, r.handleNestedFieldsForInventoryTransactions(valueCol.Field))
        }
    }
...
func (r *InventoryRepository) createWhereSQLForInventoryTransactions(request query.IServerSideGetRowsRequest) string {
    var whereParts []string

    // Handle group keys (if any)
    for i, groupKey := range request.GroupKeys {
        colName := request.RowGroupCols[i].Field
        whereParts = append(whereParts, fmt.Sprintf("%s = '%s'", r.handleNestedFieldsForInventoryTransactions(colName), groupKey))
    }

    // Handle filter model (apply filters to each column)
    for colID, filterItem := range request.FilterModel {
        whereParts = append(whereParts, r.createFilterSQLForInventoryTransactions(colID, filterItem))
    }
...
func (r *InventoryRepository) handleNestedFieldsForInventoryTransactions(field string) string {
...
func (r *InventoryRepository) createFilterSQLForInventoryTransactions(colID string, filterItem query.FilterItem) string {
	...
	case "date":
		return r.createDateFilterSQLForInventoryTransactions(colID, filterItem)
	default:
		return "true"
	}
}

func (r *InventoryRepository) createNumberFilterSQLForInventoryTransactions(colID string, filterItem query.FilterItem) string {
	...
	if filterItem.Operator != "" {
		...
		case "AND":
			...
			for _, condition := range conditions {
				andParts = append(andParts, r.createNumberFilterSQLForInventoryTransactions(colID, condition))
			}
			return strings.Join(andParts, " AND ")
		case "OR":
			var orParts []string
			for _, condition := range conditions {
				orParts = append(orParts, r.createNumberFilterSQLForInventoryTransactions(colID, condition))
			}
...
func (r *InventoryRepository) createTextFilterSQLForInventoryTransactions(colID string, filterModel query.FilterItem) string {
	// Handle filter operator and conditions (recursive)
	if filterModel.Operator != "" {
		...
		case "AND":
			var andParts []string
			for _, condition := range conditions {
				andParts = append(andParts, r.createTextFilterSQLForInventoryTransactions(colID, condition))
			}
			return strings.Join(andParts, " AND ")
		case "OR":
			var orParts []string
			for _, condition := range conditions {
				orParts = append(orParts, r.createTextFilterSQLForInventoryTransactions(colID, condition))
			}
			return strings.Join(orParts, " OR ")
		default:
			return "false"
...
func (r *InventoryRepository) createDateFilterSQLForInventoryTransactions(colID string, filterModel query.FilterItem) string {
	// Handle filter operator and conditions (recursive)
	if filterModel.Operator != "" {
		conditions := filterModel.Conditions
		switch filterModel.Operator {
		case "AND":
			var andParts []string
			for _, condition := range conditions {
				andParts = append(andParts, r.createDateFilterSQLForInventoryTransactions(colID, condition))
			}
			return strings.Join(andParts, " AND ")
		case "OR":
			var orParts []string
			for _, condition := range conditions {
				orParts = append(orParts, r.createDateFilterSQLForInventoryTransactions(colID, condition))
			}
			return strings.Join(orParts, " OR ")
		default:
			return "false"
...
func (r *InventoryRepository) createGroupBySQLForInventoryTransactions(request query.IServerSideGetRowsRequest) string {
	rowGroupCols := request.RowGroupCols
	groupKeys := request.GroupKeys
...
func (r *InventoryRepository) createOrderBySQLForInventoryTransactions(request query.IServerSideGetRowsRequest) string {
...
func (r *InventoryRepository) createLimitAndOffsetSQLForInventoryTransactions(request query.IServerSideGetRowsRequest) (limit int, offset int) {
