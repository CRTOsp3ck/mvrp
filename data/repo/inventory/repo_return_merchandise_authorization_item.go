...
func (r *InventoryRepository) createFilterSQLForReturnMerchandiseAuthorizationItems(colID string, filterItem query.FilterItem) string {
	...
	case "date":
		return r.createDateFilterSQLForReturnMerchandiseAuthorizationItems(colID, filterItem)
	default:
		return "true"
	}
}
	...
	if filterItem.Operator != "" {
		...
		case "AND":
			...
			for _, condition := range conditions {
				andParts = append(andParts, r.createNumberFilterSQLForReturnMerchandiseAuthorizationItems(colID, condition))
			}
			return strings.Join(andParts, " AND ")
		case "OR":
			var orParts []string
			for _, condition := range conditions {
				orParts = append(orParts, r.createNumberFilterSQLForReturnMerchandiseAuthorizationItems(colID, condition))
			}
...
func (r *InventoryRepository) createTextFilterSQLForReturnMerchandiseAuthorizationItems(colID string, filterModel query.FilterItem) string {
	...
	if filterModel.Operator != "" {
		...
		case "AND":
			var andParts []string
			for _, condition := range conditions {
				andParts = append(andParts, r.createTextFilterSQLForReturnMerchandiseAuthorizationItems(colID, condition))
			}
			return strings.Join(andParts, " AND ")
		case "OR":
			var orParts []string
			for _, condition := range conditions {
				orParts = append(orParts, r.createTextFilterSQLForReturnMerchandiseAuthorizationItems(colID, condition))
			}
			return strings.Join(orParts, " OR ")
		default:
			return "false"
...
func (r *InventoryRepository) createDateFilterSQLForReturnMerchandiseAuthorizationItems(colID string, filterModel query.FilterItem) string {
	// Handle filter operator and conditions (recursive)
	if filterModel.Operator != "" {
		conditions := filterModel.Conditions
		switch filterModel.Operator {
		case "AND":
			var andParts []string
			for _, condition := range conditions {
				andParts = append(andParts, r.createDateFilterSQLForReturnMerchandiseAuthorizationItems(colID, condition))
			}
			return strings.Join(andParts, " AND ")
		case "OR":
			var orParts []string
			for _, condition := range conditions {
				orParts = append(orParts, r.createDateFilterSQLForReturnMerchandiseAuthorizationItems(colID, condition))
			}
			return strings.Join(orParts, " OR ")
		default:
			return "false"
