...
func (r *InventoryRepository) createSelectSQLForInventoryViews(request query.IServerSideGetRowsRequest) string {
    ...
    if len(rowGroupCols) > len(groupKeys) {
        ...
        for i := 0; i <= len(groupKeys) && i < len(rowGroupCols); i++ {
            colsToSelect = append(colsToSelect, r.handleNestedFieldsForInventoryViews(rowGroupCols[i].Field))
        }

        // Include aggregated columns in SELECT
        for _, valueCol := range valueCols {
            colsToSelect = append(colsToSelect, fmt.Sprintf("SUM(%s) AS %s", r.handleNestedFieldsForInventoryViews(valueCol.Field), valueCol.Field))
        }
    } else {
        // If not grouping, select all columns directly
        for _, valueCol := range valueCols {
            colsToSelect = append(colsToSelect, r.handleNestedFieldsForInventoryViews(valueCol.Field))
        }
    }
...
func (r *InventoryRepository) createWhereSQLForInventoryViews(request query.IServerSideGetRowsRequest) string {
    ...
    for i, groupKey := range request.GroupKeys {
        ...
        whereParts = append(whereParts, fmt.Sprintf("%s = '%s'", r.handleNestedFieldsForInventoryViews(colName), groupKey))
    }
...
func (r *InventoryRepository) handleNestedFieldsForInventoryViews(field string) string {
...
func (r *InventoryRepository) createFilterSQLForInventoryViews(colID string, filterItem query.FilterItem) string {
	...
	case "date":
		return r.createDateFilterSQLForInventoryViews(colID, filterItem)
	default:
		return "true"
	}
}
	...
	if filterItem.Operator != "" {
		...
		case "AND":
			...
			for _, condition := range conditions {
				andParts = append(andParts, r.createNumberFilterSQLForInventoryViews(colID, condition))
			}
			return strings.Join(andParts, " AND ")
		case "OR":
			var orParts []string
			for _, condition := range conditions {
				orParts = append(orParts, r.createNumberFilterSQLForInventoryViews(colID, condition))
			}
...
func (r *InventoryRepository) createTextFilterSQLForInventoryViews(colID string, filterModel query.FilterItem) string {
	...
	if filterModel.Operator != "" {
		...
		case "AND":
			var andParts []string
			for _, condition := range conditions {
				andParts = append(andParts, r.createTextFilterSQLForInventoryViews(colID, condition))
			}
			return strings.Join(andParts, " AND ")
		case "OR":
			var orParts []string
			for _, condition := range conditions {
				orParts = append(orParts, r.createTextFilterSQLForInventoryViews(colID, condition))
			}
			return strings.Join(orParts, " OR ")
		default:
			return "false"
...
func (r *InventoryRepository) createDateFilterSQLForInventoryViews(colID string, filterModel query.FilterItem) string {
	// Handle filter operator and conditions (recursive)
	if filterModel.Operator != "" {
		conditions := filterModel.Conditions
		switch filterModel.Operator {
		case "AND":
			var andParts []string
			for _, condition := range conditions {
				andParts = append(andParts, r.createDateFilterSQLForInventoryViews(colID, condition))
			}
			return strings.Join(andParts, " AND ")
		case "OR":
			var orParts []string
			for _, condition := range conditions {
				orParts = append(orParts, r.createDateFilterSQLForInventoryViews(colID, condition))
			}
			return strings.Join(orParts, " OR ")
		default:
			return "false"
