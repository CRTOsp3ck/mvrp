...
func (r *InventoryRepository) SearchGoodsIssueNoteViews(ctx context.Context, exec boil.ContextExecutor, dto dto.SearchGoodsIssueNoteDTO) (inventory.GoodsIssueNoteViewSlice, int, error) {
	return r.BuildSearchQueryForGoodsIssueNoteViews(ctx, exec, dto)
	...
	/*
		...
		return inventory.GoodsIssueNoteViews(queryMods...).All(ctx, exec)
	*/
}
...
func (r *InventoryRepository) BuildSearchQueryForGoodsIssueNoteViews(ctx context.Context, exec boil.ContextExecutor, dto dto.SearchGoodsIssueNoteDTO) (inventory.GoodsIssueNoteViewSlice, int, error) {
	var queryMods []qm.QueryMod

	request := dto.IServerSideGetRowsRequest

	selectSQL := r.createSelectSQLForGoodsIssueNoteViews(request)
	if selectSQL != "" {
		queryMods = append(queryMods, qm.Select(selectSQL))
	}

	whereSQL := r.createWhereSQLForGoodsIssueNoteViews(request)
	if whereSQL != "" {
		queryMods = append(queryMods, qm.Where(whereSQL))
	}

	groupBySQL := r.createGroupBySQLForGoodsIssueNoteViews(request)
	if groupBySQL != "" {
		queryMods = append(queryMods, qm.GroupBy(groupBySQL))
	}

	orderBySQL := r.createOrderBySQLForGoodsIssueNoteViews(request)
	if orderBySQL != "" {
		queryMods = append(queryMods, qm.OrderBy(orderBySQL))
	}

	limitSQL, offsetSQL := r.createLimitAndOffsetSQLForGoodsIssueNoteViews(request)
	if limitSQL > 0 {
	...
	count, err := inventory.GoodsIssueNoteViews(countQueryMods...).Count(ctx, exec)
	if err != nil {
		return nil, 0, err
	}

	res, err := inventory.GoodsIssueNoteViews(queryMods...).All(ctx, exec)
	return res, int(count), err
}

// 1. Create Select SQL
func (r *InventoryRepository) createSelectSQLForGoodsIssueNoteViews(request query.IServerSideGetRowsRequest) string {
    ...
    if len(rowGroupCols) > len(groupKeys) {
        ...
        for i := 0; i <= len(groupKeys) && i < len(rowGroupCols); i++ {
            colsToSelect = append(colsToSelect, r.handleNestedFieldsForGoodsIssueNoteViews(rowGroupCols[i].Field))
        }

        // Include aggregated columns in SELECT
        for _, valueCol := range valueCols {
            colsToSelect = append(colsToSelect, fmt.Sprintf("SUM(%s) AS %s", r.handleNestedFieldsForGoodsIssueNoteViews(valueCol.Field), valueCol.Field))
        }
    } else {
        // If not grouping, select all columns directly
        for _, valueCol := range valueCols {
            colsToSelect = append(colsToSelect, r.handleNestedFieldsForGoodsIssueNoteViews(valueCol.Field))
        }
    }
...
func (r *InventoryRepository) createWhereSQLForGoodsIssueNoteViews(request query.IServerSideGetRowsRequest) string {
    var whereParts []string

    // Handle group keys (if any)
    for i, groupKey := range request.GroupKeys {
        colName := request.RowGroupCols[i].Field
        whereParts = append(whereParts, fmt.Sprintf("%s = '%s'", r.handleNestedFieldsForGoodsIssueNoteViews(colName), groupKey))
    }

    // Handle filter model (apply filters to each column)
    for colID, filterItem := range request.FilterModel {
        whereParts = append(whereParts, r.createFilterSQLForGoodsIssueNoteViews(colID, filterItem))
    }
...
func (r *InventoryRepository) handleNestedFieldsForGoodsIssueNoteViews(field string) string {
...
func (r *InventoryRepository) createFilterSQLForGoodsIssueNoteViews(colID string, filterItem query.FilterItem) string {
	...
	case "text":
		return r.createTextFilterSQLForGoodsIssueNoteViews(colID, filterItem)
	case "number":
		// Cast the JSONB text value to a number
		if strings.Contains(colID, "->>'") {
			colID = fmt.Sprintf("(%s)::numeric", colID)
		}
		return r.createNumberFilterSQLForGoodsIssueNoteViews(colID, filterItem)
	case "date":
		return r.createDateFilterSQLForGoodsIssueNoteViews(colID, filterItem)
	default:
		return "true"
	}
}

func (r *InventoryRepository) createNumberFilterSQLForGoodsIssueNoteViews(colID string, filterItem query.FilterItem) string {
	...
	if filterItem.Operator != "" {
		...
		case "AND":
			...
			for _, condition := range conditions {
				andParts = append(andParts, r.createNumberFilterSQLForGoodsIssueNoteViews(colID, condition))
			}
			return strings.Join(andParts, " AND ")
		case "OR":
			var orParts []string
			for _, condition := range conditions {
				orParts = append(orParts, r.createNumberFilterSQLForGoodsIssueNoteViews(colID, condition))
			}
...
func (r *InventoryRepository) createTextFilterSQLForGoodsIssueNoteViews(colID string, filterModel query.FilterItem) string {
	// Handle filter operator and conditions (recursive)
	if filterModel.Operator != "" {
		...
		case "AND":
			var andParts []string
			for _, condition := range conditions {
				andParts = append(andParts, r.createTextFilterSQLForGoodsIssueNoteViews(colID, condition))
			}
			return strings.Join(andParts, " AND ")
		case "OR":
			var orParts []string
			for _, condition := range conditions {
				orParts = append(orParts, r.createTextFilterSQLForGoodsIssueNoteViews(colID, condition))
			}
			return strings.Join(orParts, " OR ")
		default:
			return "false"
...
func (r *InventoryRepository) createDateFilterSQLForGoodsIssueNoteViews(colID string, filterModel query.FilterItem) string {
	// Handle filter operator and conditions (recursive)
	if filterModel.Operator != "" {
		conditions := filterModel.Conditions
		switch filterModel.Operator {
		case "AND":
			var andParts []string
			for _, condition := range conditions {
				andParts = append(andParts, r.createDateFilterSQLForGoodsIssueNoteViews(colID, condition))
			}
			return strings.Join(andParts, " AND ")
		case "OR":
			var orParts []string
			for _, condition := range conditions {
				orParts = append(orParts, r.createDateFilterSQLForGoodsIssueNoteViews(colID, condition))
			}
			return strings.Join(orParts, " OR ")
		default:
			return "false"
...
func (r *InventoryRepository) createGroupBySQLForGoodsIssueNoteViews(request query.IServerSideGetRowsRequest) string {
	rowGroupCols := request.RowGroupCols
	groupKeys := request.GroupKeys
...
func (r *InventoryRepository) createOrderBySQLForGoodsIssueNoteViews(request query.IServerSideGetRowsRequest) string {
...
func (r *InventoryRepository) createLimitAndOffsetSQLForGoodsIssueNoteViews(request query.IServerSideGetRowsRequest) (limit int, offset int) {
