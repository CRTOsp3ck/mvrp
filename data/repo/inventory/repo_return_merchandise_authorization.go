...
func (r *InventoryRepository) GetNextEntryReturnMerchandiseAuthorizationID(ctx context.Context, exec boil.ContextExecutor) (int, error) {
	...
	err := inventory.ReturnMerchandiseAuthorizations(qm.Select("MAX(id)")).QueryRow(exec).Scan(&maxID)
	if err != nil {
		return 0, err
	}
...
func (r *InventoryRepository) SearchReturnMerchandiseAuthorizations(ctx context.Context, exec boil.ContextExecutor, dto dto.SearchReturnMerchandiseAuthorizationDTO) (inventory.ReturnMerchandiseAuthorizationSlice, int, error) {
	return r.BuildSearchQueryForReturnMerchandiseAuthorizations(ctx, exec, dto)
	...
	/*
		...
		return inventory.ReturnMerchandiseAuthorizations(queryMods...).All(ctx, exec)
	*/
}
...
func (r *InventoryRepository) BuildSearchQueryForReturnMerchandiseAuthorizations(ctx context.Context, exec boil.ContextExecutor, dto dto.SearchReturnMerchandiseAuthorizationDTO) (inventory.ReturnMerchandiseAuthorizationSlice, int, error) {
	var queryMods []qm.QueryMod

	request := dto.IServerSideGetRowsRequest

	selectSQL := r.createSelectSQLForReturnMerchandiseAuthorizations(request)
	if selectSQL != "" {
		queryMods = append(queryMods, qm.Select(selectSQL))
	}

	whereSQL := r.createWhereSQLForReturnMerchandiseAuthorizations(request)
	if whereSQL != "" {
		queryMods = append(queryMods, qm.Where(whereSQL))
	}

	groupBySQL := r.createGroupBySQLForReturnMerchandiseAuthorizations(request)
	if groupBySQL != "" {
		queryMods = append(queryMods, qm.GroupBy(groupBySQL))
	}

	orderBySQL := r.createOrderBySQLForReturnMerchandiseAuthorizations(request)
	if orderBySQL != "" {
		queryMods = append(queryMods, qm.OrderBy(orderBySQL))
	}

	limitSQL, offsetSQL := r.createLimitAndOffsetSQLForReturnMerchandiseAuthorizations(request)
	if limitSQL > 0 {
	...
	count, err := inventory.ReturnMerchandiseAuthorizations(countQueryMods...).Count(ctx, exec)
	if err != nil {
		return nil, 0, err
	}

	res, err := inventory.ReturnMerchandiseAuthorizations(queryMods...).All(ctx, exec)
	return res, int(count), err
}

// 1. Create Select SQL
func (r *InventoryRepository) createSelectSQLForReturnMerchandiseAuthorizations(request query.IServerSideGetRowsRequest) string {
    ...
    if len(rowGroupCols) > len(groupKeys) {
        ...
        for i := 0; i <= len(groupKeys) && i < len(rowGroupCols); i++ {
            colsToSelect = append(colsToSelect, r.handleNestedFieldsForReturnMerchandiseAuthorizations(rowGroupCols[i].Field))
        }

        // Include aggregated columns in SELECT
        for _, valueCol := range valueCols {
            colsToSelect = append(colsToSelect, fmt.Sprintf("SUM(%s) AS %s", r.handleNestedFieldsForReturnMerchandiseAuthorizations(valueCol.Field), valueCol.Field))
        }
    } else {
        // If not grouping, select all columns directly
        for _, valueCol := range valueCols {
            colsToSelect = append(colsToSelect, r.handleNestedFieldsForReturnMerchandiseAuthorizations(valueCol.Field))
        }
    }
...
func (r *InventoryRepository) createWhereSQLForReturnMerchandiseAuthorizations(request query.IServerSideGetRowsRequest) string {
    var whereParts []string

    // Handle group keys (if any)
    for i, groupKey := range request.GroupKeys {
        colName := request.RowGroupCols[i].Field
        whereParts = append(whereParts, fmt.Sprintf("%s = '%s'", r.handleNestedFieldsForReturnMerchandiseAuthorizations(colName), groupKey))
    }

    // Handle filter model (apply filters to each column)
    for colID, filterItem := range request.FilterModel {
        whereParts = append(whereParts, r.createFilterSQLForReturnMerchandiseAuthorizations(colID, filterItem))
    }
...
func (r *InventoryRepository) handleNestedFieldsForReturnMerchandiseAuthorizations(field string) string {
...
func (r *InventoryRepository) createFilterSQLForReturnMerchandiseAuthorizations(colID string, filterItem query.FilterItem) string {
	...
	case "text":
		return r.createTextFilterSQLForReturnMerchandiseAuthorizations(colID, filterItem)
	case "number":
		// Cast the JSONB text value to a number
		if strings.Contains(colID, "->>'") {
			colID = fmt.Sprintf("(%s)::numeric", colID)
		}
		return r.createNumberFilterSQLForReturnMerchandiseAuthorizations(colID, filterItem)
	case "date":
		return r.createDateFilterSQLForReturnMerchandiseAuthorizations(colID, filterItem)
	default:
		return "true"
	}
}

func (r *InventoryRepository) createNumberFilterSQLForReturnMerchandiseAuthorizations(colID string, filterItem query.FilterItem) string {
	...
	if filterItem.Operator != "" {
		...
		case "AND":
			...
			for _, condition := range conditions {
				andParts = append(andParts, r.createNumberFilterSQLForReturnMerchandiseAuthorizations(colID, condition))
			}
			return strings.Join(andParts, " AND ")
		case "OR":
			var orParts []string
			for _, condition := range conditions {
				orParts = append(orParts, r.createNumberFilterSQLForReturnMerchandiseAuthorizations(colID, condition))
			}
...
func (r *InventoryRepository) createTextFilterSQLForReturnMerchandiseAuthorizations(colID string, filterModel query.FilterItem) string {
	// Handle filter operator and conditions (recursive)
	if filterModel.Operator != "" {
		...
		case "AND":
			var andParts []string
			for _, condition := range conditions {
				andParts = append(andParts, r.createTextFilterSQLForReturnMerchandiseAuthorizations(colID, condition))
			}
			return strings.Join(andParts, " AND ")
		case "OR":
			var orParts []string
			for _, condition := range conditions {
				orParts = append(orParts, r.createTextFilterSQLForReturnMerchandiseAuthorizations(colID, condition))
			}
			return strings.Join(orParts, " OR ")
		default:
			return "false"
...
func (r *InventoryRepository) createDateFilterSQLForReturnMerchandiseAuthorizations(colID string, filterModel query.FilterItem) string {
	// Handle filter operator and conditions (recursive)
	if filterModel.Operator != "" {
		conditions := filterModel.Conditions
		switch filterModel.Operator {
		case "AND":
			var andParts []string
			for _, condition := range conditions {
				andParts = append(andParts, r.createDateFilterSQLForReturnMerchandiseAuthorizations(colID, condition))
			}
			return strings.Join(andParts, " AND ")
		case "OR":
			var orParts []string
			for _, condition := range conditions {
				orParts = append(orParts, r.createDateFilterSQLForReturnMerchandiseAuthorizations(colID, condition))
			}
			return strings.Join(orParts, " OR ")
		default:
			return "false"
...
func (r *InventoryRepository) createGroupBySQLForReturnMerchandiseAuthorizations(request query.IServerSideGetRowsRequest) string {
	rowGroupCols := request.RowGroupCols
	groupKeys := request.GroupKeys
...
func (r *InventoryRepository) createOrderBySQLForReturnMerchandiseAuthorizations(request query.IServerSideGetRowsRequest) string {
...
func (r *InventoryRepository) createLimitAndOffsetSQLForReturnMerchandiseAuthorizations(request query.IServerSideGetRowsRequest) (limit int, offset int) {
