...
func (r *SaleRepository) createFilterSQLForOrderConfirmations(colID string, filterItem query.FilterItem) string {
	...
	case "date":
		return r.createDateFilterSQLForOrderConfirmations(colID, filterItem)
	default:
		return "true"
	}
}
...
func (r *SaleRepository) createDateFilterSQLForOrderConfirmations(colID string, filterModel query.FilterItem) string {
	// Handle filter operator and conditions (recursive)
	if filterModel.Operator != "" {
		conditions := filterModel.Conditions
		switch filterModel.Operator {
		case "AND":
			var andParts []string
			for _, condition := range conditions {
				andParts = append(andParts, r.createDateFilterSQLForOrderConfirmations(colID, condition))
			}
			return strings.Join(andParts, " AND ")
		case "OR":
			var orParts []string
			for _, condition := range conditions {
				orParts = append(orParts, r.createDateFilterSQLForOrderConfirmations(colID, condition))
			}
			return strings.Join(orParts, " OR ")
		default:
			return "false"
