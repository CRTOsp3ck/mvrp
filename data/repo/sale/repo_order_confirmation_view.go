...
func (r *SaleRepository) createFilterSQLForOrderConfirmationViews(colID string, filterItem query.FilterItem) string {
	...
	case "date":
		return r.createDateFilterSQLForOrderConfirmationViews(colID, filterItem)
	default:
		return "true"
	}
}
	...
	if filterItem.Operator != "" {
		...
		case "AND":
			...
			for _, condition := range conditions {
				andParts = append(andParts, r.createNumberFilterSQLForOrderConfirmationViews(colID, condition))
			}
			return strings.Join(andParts, " AND ")
		case "OR":
			var orParts []string
			for _, condition := range conditions {
				orParts = append(orParts, r.createNumberFilterSQLForOrderConfirmationViews(colID, condition))
			}
...
func (r *SaleRepository) createTextFilterSQLForOrderConfirmationViews(colID string, filterModel query.FilterItem) string {
	...
	if filterModel.Operator != "" {
		...
		case "AND":
			var andParts []string
			for _, condition := range conditions {
				andParts = append(andParts, r.createTextFilterSQLForOrderConfirmationViews(colID, condition))
			}
			return strings.Join(andParts, " AND ")
		case "OR":
			var orParts []string
			for _, condition := range conditions {
				orParts = append(orParts, r.createTextFilterSQLForOrderConfirmationViews(colID, condition))
			}
			return strings.Join(orParts, " OR ")
		default:
			return "false"
...
func (r *SaleRepository) createDateFilterSQLForOrderConfirmationViews(colID string, filterModel query.FilterItem) string {
	// Handle filter operator and conditions (recursive)
	if filterModel.Operator != "" {
		conditions := filterModel.Conditions
		switch filterModel.Operator {
		case "AND":
			var andParts []string
			for _, condition := range conditions {
				andParts = append(andParts, r.createDateFilterSQLForOrderConfirmationViews(colID, condition))
			}
			return strings.Join(andParts, " AND ")
		case "OR":
			var orParts []string
			for _, condition := range conditions {
				orParts = append(orParts, r.createDateFilterSQLForOrderConfirmationViews(colID, condition))
			}
			return strings.Join(orParts, " OR ")
		default:
			return "false"
