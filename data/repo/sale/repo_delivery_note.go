...
func (r *SaleRepository) GetNextEntryDeliveryNoteID(ctx context.Context, exec boil.ContextExecutor) (int, error) {
	...
	err := sale.DeliveryNotes(qm.Select("MAX(id)")).QueryRow(exec).Scan(&maxID)
	if err != nil {
		return 0, err
	}
...
func (r *SaleRepository) SearchDeliveryNotes(ctx context.Context, exec boil.ContextExecutor, dto dto.SearchDeliveryNoteDTO) (sale.DeliveryNoteSlice, int, error) {
	return r.BuildSearchQueryForDeliveryNotes(ctx, exec, dto)
...
func (r *SaleRepository) BuildSearchQueryForDeliveryNotes(ctx context.Context, exec boil.ContextExecutor, dto dto.SearchDeliveryNoteDTO) (sale.DeliveryNoteSlice, int, error) {
	var queryMods []qm.QueryMod
	if dto.ShippingStatus != "" {
		queryMods = append(queryMods, qm.Where("shipping_status = ?", dto.ShippingStatus))
	}
	...
	selectSQL := r.createSelectSQLForDeliveryNotes(request)
	if selectSQL != "" {
		queryMods = append(queryMods, qm.Select(selectSQL))
	}

	whereSQL := r.createWhereSQLForDeliveryNotes(request)
	if whereSQL != "" {
		queryMods = append(queryMods, qm.Where(whereSQL))
	}

	groupBySQL := r.createGroupBySQLForDeliveryNotes(request)
	if groupBySQL != "" {
		queryMods = append(queryMods, qm.GroupBy(groupBySQL))
	}

	orderBySQL := r.createOrderBySQLForDeliveryNotes(request)
	if orderBySQL != "" {
		queryMods = append(queryMods, qm.OrderBy(orderBySQL))
	}

	limitSQL, offsetSQL := r.createLimitAndOffsetSQLForDeliveryNotes(request)
	if limitSQL > 0 {
	...
	count, err := sale.DeliveryNotes(countQueryMods...).Count(ctx, exec)
	if err != nil {
		return nil, 0, err
	}

	res, err := sale.DeliveryNotes(queryMods...).All(ctx, exec)
	return res, int(count), err
}

// 1. Create Select SQL
func (r *SaleRepository) createSelectSQLForDeliveryNotes(request query.IServerSideGetRowsRequest) string {
    ...
    if len(rowGroupCols) > len(groupKeys) {
        ...
        for i := 0; i <= len(groupKeys) && i < len(rowGroupCols); i++ {
            colsToSelect = append(colsToSelect, r.handleNestedFieldsForDeliveryNotes(rowGroupCols[i].Field))
        }

        // Include aggregated columns in SELECT
        for _, valueCol := range valueCols {
            colsToSelect = append(colsToSelect, fmt.Sprintf("SUM(%s) AS %s", r.handleNestedFieldsForDeliveryNotes(valueCol.Field), valueCol.Field))
        }
    } else {
        // If not grouping, select all columns directly
        for _, valueCol := range valueCols {
            colsToSelect = append(colsToSelect, r.handleNestedFieldsForDeliveryNotes(valueCol.Field))
        }
    }
...
func (r *SaleRepository) createWhereSQLForDeliveryNotes(request query.IServerSideGetRowsRequest) string {
    var whereParts []string

    // Handle group keys (if any)
    for i, groupKey := range request.GroupKeys {
        colName := request.RowGroupCols[i].Field
        whereParts = append(whereParts, fmt.Sprintf("%s = '%s'", r.handleNestedFieldsForDeliveryNotes(colName), groupKey))
    }

    // Handle filter model (apply filters to each column)
    for colID, filterItem := range request.FilterModel {
        whereParts = append(whereParts, r.createFilterSQLForDeliveryNotes(colID, filterItem))
    }
...
func (r *SaleRepository) handleNestedFieldsForDeliveryNotes(field string) string {
...
func (r *SaleRepository) createFilterSQLForDeliveryNotes(colID string, filterItem query.FilterItem) string {
	...
	case "text":
		return r.createTextFilterSQLForDeliveryNotes(colID, filterItem)
	case "number":
		// Cast the JSONB text value to a number
		if strings.Contains(colID, "->>'") {
			colID = fmt.Sprintf("(%s)::numeric", colID)
		}
		return r.createNumberFilterSQLForDeliveryNotes(colID, filterItem)
	case "date":
		return r.createDateFilterSQLForDeliveryNotes(colID, filterItem)
	default:
		return "true"
	}
}

func (r *SaleRepository) createNumberFilterSQLForDeliveryNotes(colID string, filterItem query.FilterItem) string {
	...
	if filterItem.Operator != "" {
		...
		case "AND":
			...
			for _, condition := range conditions {
				andParts = append(andParts, r.createNumberFilterSQLForDeliveryNotes(colID, condition))
			}
			return strings.Join(andParts, " AND ")
		case "OR":
			var orParts []string
			for _, condition := range conditions {
				orParts = append(orParts, r.createNumberFilterSQLForDeliveryNotes(colID, condition))
			}
...
func (r *SaleRepository) createTextFilterSQLForDeliveryNotes(colID string, filterModel query.FilterItem) string {
	// Handle filter operator and conditions (recursive)
	if filterModel.Operator != "" {
		...
		case "AND":
			var andParts []string
			for _, condition := range conditions {
				andParts = append(andParts, r.createTextFilterSQLForDeliveryNotes(colID, condition))
			}
			return strings.Join(andParts, " AND ")
		case "OR":
			var orParts []string
			for _, condition := range conditions {
				orParts = append(orParts, r.createTextFilterSQLForDeliveryNotes(colID, condition))
			}
			return strings.Join(orParts, " OR ")
		default:
			return "false"
...
func (r *SaleRepository) createDateFilterSQLForDeliveryNotes(colID string, filterModel query.FilterItem) string {
	// Handle filter operator and conditions (recursive)
	if filterModel.Operator != "" {
		conditions := filterModel.Conditions
		switch filterModel.Operator {
		case "AND":
			var andParts []string
			for _, condition := range conditions {
				andParts = append(andParts, r.createDateFilterSQLForDeliveryNotes(colID, condition))
			}
			return strings.Join(andParts, " AND ")
		case "OR":
			var orParts []string
			for _, condition := range conditions {
				orParts = append(orParts, r.createDateFilterSQLForDeliveryNotes(colID, condition))
			}
			return strings.Join(orParts, " OR ")
		default:
			return "false"
...
func (r *SaleRepository) createGroupBySQLForDeliveryNotes(request query.IServerSideGetRowsRequest) string {
	rowGroupCols := request.RowGroupCols
	groupKeys := request.GroupKeys
...
func (r *SaleRepository) createOrderBySQLForDeliveryNotes(request query.IServerSideGetRowsRequest) string {
...
func (r *SaleRepository) createLimitAndOffsetSQLForDeliveryNotes(request query.IServerSideGetRowsRequest) (limit int, offset int) {
