...
func (r *SaleRepository) SearchGoodsReturnNoteViews(ctx context.Context, exec boil.ContextExecutor, dto dto.SearchGoodsReturnNoteDTO) (sale.GoodsReturnNoteViewSlice, int, error) {
	return r.BuildSearchQueryForGoodsReturnNoteViews(ctx, exec, dto)
...
func (r *SaleRepository) BuildSearchQueryForGoodsReturnNoteViews(ctx context.Context, exec boil.ContextExecutor, dto dto.SearchGoodsReturnNoteDTO) (sale.GoodsReturnNoteViewSlice, int, error) {
	var queryMods []qm.QueryMod

	request := dto.IServerSideGetRowsRequest

	selectSQL := r.createSelectSQLForGoodsReturnNoteViews(request)
	if selectSQL != "" {
		queryMods = append(queryMods, qm.Select(selectSQL))
	}

	whereSQL := r.createWhereSQLForGoodsReturnNoteViews(request)
	if whereSQL != "" {
		queryMods = append(queryMods, qm.Where(whereSQL))
	}

	groupBySQL := r.createGroupBySQLForGoodsReturnNoteViews(request)
	if groupBySQL != "" {
		queryMods = append(queryMods, qm.GroupBy(groupBySQL))
	}

	orderBySQL := r.createOrderBySQLForGoodsReturnNoteViews(request)
	if orderBySQL != "" {
		queryMods = append(queryMods, qm.OrderBy(orderBySQL))
	}

	limitSQL, offsetSQL := r.createLimitAndOffsetSQLForGoodsReturnNoteViews(request)
	if limitSQL > 0 {
	...
	count, err := sale.GoodsReturnNoteViews(countQueryMods...).Count(ctx, exec)
	if err != nil {
		return nil, 0, err
	}

	res, err := sale.GoodsReturnNoteViews(queryMods...).All(ctx, exec)
	return res, int(count), err
}

// 1. Create Select SQL
func (r *SaleRepository) createSelectSQLForGoodsReturnNoteViews(request query.IServerSideGetRowsRequest) string {
    ...
    if len(rowGroupCols) > len(groupKeys) {
        ...
        for i := 0; i <= len(groupKeys) && i < len(rowGroupCols); i++ {
            colsToSelect = append(colsToSelect, r.handleNestedFieldsForGoodsReturnNoteViews(rowGroupCols[i].Field))
        }

        // Include aggregated columns in SELECT
        for _, valueCol := range valueCols {
            colsToSelect = append(colsToSelect, fmt.Sprintf("SUM(%s) AS %s", r.handleNestedFieldsForGoodsReturnNoteViews(valueCol.Field), valueCol.Field))
        }
    } else {
        // If not grouping, select all columns directly
        for _, valueCol := range valueCols {
            colsToSelect = append(colsToSelect, r.handleNestedFieldsForGoodsReturnNoteViews(valueCol.Field))
        }
    }
...
func (r *SaleRepository) createWhereSQLForGoodsReturnNoteViews(request query.IServerSideGetRowsRequest) string {
    var whereParts []string

    // Handle group keys (if any)
    for i, groupKey := range request.GroupKeys {
        colName := request.RowGroupCols[i].Field
        whereParts = append(whereParts, fmt.Sprintf("%s = '%s'", r.handleNestedFieldsForGoodsReturnNoteViews(colName), groupKey))
    }

    // Handle filter model (apply filters to each column)
    for colID, filterItem := range request.FilterModel {
        whereParts = append(whereParts, r.createFilterSQLForGoodsReturnNoteViews(colID, filterItem))
    }
...
func (r *SaleRepository) handleNestedFieldsForGoodsReturnNoteViews(field string) string {
...
func (r *SaleRepository) createFilterSQLForGoodsReturnNoteViews(colID string, filterItem query.FilterItem) string {
	...
	case "text":
		return r.createTextFilterSQLForGoodsReturnNoteViews(colID, filterItem)
	case "number":
		// Cast the JSONB text value to a number
		if strings.Contains(colID, "->>'") {
			colID = fmt.Sprintf("(%s)::numeric", colID)
		}
		return r.createNumberFilterSQLForGoodsReturnNoteViews(colID, filterItem)
	case "date":
		return r.createDateFilterSQLForGoodsReturnNoteViews(colID, filterItem)
	default:
		return "true"
	}
}

func (r *SaleRepository) createNumberFilterSQLForGoodsReturnNoteViews(colID string, filterItem query.FilterItem) string {
	...
	if filterItem.Operator != "" {
		...
		case "AND":
			...
			for _, condition := range conditions {
				andParts = append(andParts, r.createNumberFilterSQLForGoodsReturnNoteViews(colID, condition))
			}
			return strings.Join(andParts, " AND ")
		case "OR":
			var orParts []string
			for _, condition := range conditions {
				orParts = append(orParts, r.createNumberFilterSQLForGoodsReturnNoteViews(colID, condition))
			}
...
func (r *SaleRepository) createTextFilterSQLForGoodsReturnNoteViews(colID string, filterModel query.FilterItem) string {
	// Handle filter operator and conditions (recursive)
	if filterModel.Operator != "" {
		...
		case "AND":
			var andParts []string
			for _, condition := range conditions {
				andParts = append(andParts, r.createTextFilterSQLForGoodsReturnNoteViews(colID, condition))
			}
			return strings.Join(andParts, " AND ")
		case "OR":
			var orParts []string
			for _, condition := range conditions {
				orParts = append(orParts, r.createTextFilterSQLForGoodsReturnNoteViews(colID, condition))
			}
			return strings.Join(orParts, " OR ")
		default:
			return "false"
...
func (r *SaleRepository) createDateFilterSQLForGoodsReturnNoteViews(colID string, filterModel query.FilterItem) string {
	// Handle filter operator and conditions (recursive)
	if filterModel.Operator != "" {
		conditions := filterModel.Conditions
		switch filterModel.Operator {
		case "AND":
			var andParts []string
			for _, condition := range conditions {
				andParts = append(andParts, r.createDateFilterSQLForGoodsReturnNoteViews(colID, condition))
			}
			return strings.Join(andParts, " AND ")
		case "OR":
			var orParts []string
			for _, condition := range conditions {
				orParts = append(orParts, r.createDateFilterSQLForGoodsReturnNoteViews(colID, condition))
			}
			return strings.Join(orParts, " OR ")
		default:
			return "false"
...
func (r *SaleRepository) createGroupBySQLForGoodsReturnNoteViews(request query.IServerSideGetRowsRequest) string {
	rowGroupCols := request.RowGroupCols
	groupKeys := request.GroupKeys
...
func (r *SaleRepository) createOrderBySQLForGoodsReturnNoteViews(request query.IServerSideGetRowsRequest) string {
...
func (r *SaleRepository) createLimitAndOffsetSQLForGoodsReturnNoteViews(request query.IServerSideGetRowsRequest) (limit int, offset int) {
