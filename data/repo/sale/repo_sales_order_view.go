...
func (r *SaleRepository) createSelectSQLForSalesOrderViews(request query.IServerSideGetRowsRequest) string {
    ...
    if len(rowGroupCols) > len(groupKeys) {
        ...
        for i := 0; i <= len(groupKeys) && i < len(rowGroupCols); i++ {
            colsToSelect = append(colsToSelect, r.handleNestedFieldsForSalesOrderViews(rowGroupCols[i].Field))
        }

        // Include aggregated columns in SELECT
        for _, valueCol := range valueCols {
            colsToSelect = append(colsToSelect, fmt.Sprintf("SUM(%s) AS %s", r.handleNestedFieldsForSalesOrderViews(valueCol.Field), valueCol.Field))
        }
    } else {
        // If not grouping, select all columns directly
        for _, valueCol := range valueCols {
            colsToSelect = append(colsToSelect, r.handleNestedFieldsForSalesOrderViews(valueCol.Field))
        }
    }
...
func (r *SaleRepository) createWhereSQLForSalesOrderViews(request query.IServerSideGetRowsRequest) string {
    ...
    for i, groupKey := range request.GroupKeys {
        ...
        whereParts = append(whereParts, fmt.Sprintf("%s = '%s'", r.handleNestedFieldsForSalesOrderViews(colName), groupKey))
    }
...
func (r *SaleRepository) handleNestedFieldsForSalesOrderViews(field string) string {
...
func (r *SaleRepository) createFilterSQLForSalesOrderViews(colID string, filterItem query.FilterItem) string {
	...
	case "date":
		return r.createDateFilterSQLForSalesOrderViews(colID, filterItem)
	default:
		return "true"
	}
}
...
func (r *SaleRepository) createDateFilterSQLForSalesOrderViews(colID string, filterModel query.FilterItem) string {
	// Handle filter operator and conditions (recursive)
	if filterModel.Operator != "" {
		conditions := filterModel.Conditions
		switch filterModel.Operator {
		case "AND":
			var andParts []string
			for _, condition := range conditions {
				andParts = append(andParts, r.createDateFilterSQLForSalesOrderViews(colID, condition))
			}
			return strings.Join(andParts, " AND ")
		case "OR":
			var orParts []string
			for _, condition := range conditions {
				orParts = append(orParts, r.createDateFilterSQLForSalesOrderViews(colID, condition))
			}
			return strings.Join(orParts, " OR ")
		default:
			return "false"
