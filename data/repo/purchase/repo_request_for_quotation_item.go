...
func (r *PurchaseRepository) createFilterSQLForRequestForQuotationItems(colID string, filterItem query.FilterItem) string {
	...
	case "date":
		return r.createDateFilterSQLForRequestForQuotationItems(colID, filterItem)
	default:
		return "true"
	}
}
...
func (r *PurchaseRepository) createDateFilterSQLForRequestForQuotationItems(colID string, filterModel query.FilterItem) string {
	// Handle filter operator and conditions (recursive)
	if filterModel.Operator != "" {
		conditions := filterModel.Conditions
		switch filterModel.Operator {
		case "AND":
			var andParts []string
			for _, condition := range conditions {
				andParts = append(andParts, r.createDateFilterSQLForRequestForQuotationItems(colID, condition))
			}
			return strings.Join(andParts, " AND ")
		case "OR":
			var orParts []string
			for _, condition := range conditions {
				orParts = append(orParts, r.createDateFilterSQLForRequestForQuotationItems(colID, condition))
			}
			return strings.Join(orParts, " OR ")
		default:
			return "false"
