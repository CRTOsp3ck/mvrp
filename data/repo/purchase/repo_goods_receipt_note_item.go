...
func (r *PurchaseRepository) SearchGoodsReceiptNoteItems(ctx context.Context, exec boil.ContextExecutor, dto dto.SearchGoodsReceiptNoteItemDTO) (purchase.GoodsReceiptNoteItemSlice, int, error) {
	return r.BuildSearchQueryForGoodsReceiptNoteItems(ctx, exec, dto)
...
func (r *PurchaseRepository) BuildSearchQueryForGoodsReceiptNoteItems(ctx context.Context, exec boil.ContextExecutor, dto dto.SearchGoodsReceiptNoteItemDTO) (purchase.GoodsReceiptNoteItemSlice, int, error) {
	var queryMods []qm.QueryMod

	request := dto.IServerSideGetRowsRequest

	selectSQL := r.createSelectSQLForGoodsReceiptNoteItems(request)
	if selectSQL != "" {
		queryMods = append(queryMods, qm.Select(selectSQL))
	}

	whereSQL := r.createWhereSQLForGoodsReceiptNoteItems(request)
	if whereSQL != "" {
		queryMods = append(queryMods, qm.Where(whereSQL))
	}

	groupBySQL := r.createGroupBySQLForGoodsReceiptNoteItems(request)
	if groupBySQL != "" {
		queryMods = append(queryMods, qm.GroupBy(groupBySQL))
	}

	orderBySQL := r.createOrderBySQLForGoodsReceiptNoteItems(request)
	if orderBySQL != "" {
		queryMods = append(queryMods, qm.OrderBy(orderBySQL))
	}

	limitSQL, offsetSQL := r.createLimitAndOffsetSQLForGoodsReceiptNoteItems(request)
	if limitSQL > 0 {
	...
	count, err := purchase.GoodsReceiptNoteItems(countQueryMods...).Count(ctx, exec)
	if err != nil {
		return nil, 0, err
	}

	res, err := purchase.GoodsReceiptNoteItems(queryMods...).All(ctx, exec)
	return res, int(count), err
}

// 1. Create Select SQL
func (r *PurchaseRepository) createSelectSQLForGoodsReceiptNoteItems(request query.IServerSideGetRowsRequest) string {
    ...
    if len(rowGroupCols) > len(groupKeys) {
        ...
        for i := 0; i <= len(groupKeys) && i < len(rowGroupCols); i++ {
            colsToSelect = append(colsToSelect, r.handleNestedFieldsForGoodsReceiptNoteItems(rowGroupCols[i].Field))
        }

        // Include aggregated columns in SELECT
        for _, valueCol := range valueCols {
            colsToSelect = append(colsToSelect, fmt.Sprintf("SUM(%s) AS %s", r.handleNestedFieldsForGoodsReceiptNoteItems(valueCol.Field), valueCol.Field))
        }
    } else {
        // If not grouping, select all columns directly
        for _, valueCol := range valueCols {
            colsToSelect = append(colsToSelect, r.handleNestedFieldsForGoodsReceiptNoteItems(valueCol.Field))
        }
    }
...
func (r *PurchaseRepository) createWhereSQLForGoodsReceiptNoteItems(request query.IServerSideGetRowsRequest) string {
    var whereParts []string

    // Handle group keys (if any)
    for i, groupKey := range request.GroupKeys {
        colName := request.RowGroupCols[i].Field
        whereParts = append(whereParts, fmt.Sprintf("%s = '%s'", r.handleNestedFieldsForGoodsReceiptNoteItems(colName), groupKey))
    }

    // Handle filter model (apply filters to each column)
    for colID, filterItem := range request.FilterModel {
        whereParts = append(whereParts, r.createFilterSQLForGoodsReceiptNoteItems(colID, filterItem))
    }
...
func (r *PurchaseRepository) handleNestedFieldsForGoodsReceiptNoteItems(field string) string {
...
func (r *PurchaseRepository) createFilterSQLForGoodsReceiptNoteItems(colID string, filterItem query.FilterItem) string {
	...
	case "text":
		return r.createTextFilterSQLForGoodsReceiptNoteItems(colID, filterItem)
	case "number":
		// Cast the JSONB text value to a number
		if strings.Contains(colID, "->>'") {
			colID = fmt.Sprintf("(%s)::numeric", colID)
		}
		return r.createNumberFilterSQLForGoodsReceiptNoteItems(colID, filterItem)
	case "date":
		return r.createDateFilterSQLForGoodsReceiptNoteItems(colID, filterItem)
	default:
		return "true"
	}
}

func (r *PurchaseRepository) createNumberFilterSQLForGoodsReceiptNoteItems(colID string, filterItem query.FilterItem) string {
	...
	if filterItem.Operator != "" {
		...
		case "AND":
			...
			for _, condition := range conditions {
				andParts = append(andParts, r.createNumberFilterSQLForGoodsReceiptNoteItems(colID, condition))
			}
			return strings.Join(andParts, " AND ")
		case "OR":
			var orParts []string
			for _, condition := range conditions {
				orParts = append(orParts, r.createNumberFilterSQLForGoodsReceiptNoteItems(colID, condition))
			}
...
func (r *PurchaseRepository) createTextFilterSQLForGoodsReceiptNoteItems(colID string, filterModel query.FilterItem) string {
	// Handle filter operator and conditions (recursive)
	if filterModel.Operator != "" {
		...
		case "AND":
			var andParts []string
			for _, condition := range conditions {
				andParts = append(andParts, r.createTextFilterSQLForGoodsReceiptNoteItems(colID, condition))
			}
			return strings.Join(andParts, " AND ")
		case "OR":
			var orParts []string
			for _, condition := range conditions {
				orParts = append(orParts, r.createTextFilterSQLForGoodsReceiptNoteItems(colID, condition))
			}
			return strings.Join(orParts, " OR ")
		default:
			return "false"
...
func (r *PurchaseRepository) createDateFilterSQLForGoodsReceiptNoteItems(colID string, filterModel query.FilterItem) string {
	// Handle filter operator and conditions (recursive)
	if filterModel.Operator != "" {
		conditions := filterModel.Conditions
		switch filterModel.Operator {
		case "AND":
			var andParts []string
			for _, condition := range conditions {
				andParts = append(andParts, r.createDateFilterSQLForGoodsReceiptNoteItems(colID, condition))
			}
			return strings.Join(andParts, " AND ")
		case "OR":
			var orParts []string
			for _, condition := range conditions {
				orParts = append(orParts, r.createDateFilterSQLForGoodsReceiptNoteItems(colID, condition))
			}
			return strings.Join(orParts, " OR ")
		default:
			return "false"
...
func (r *PurchaseRepository) createGroupBySQLForGoodsReceiptNoteItems(request query.IServerSideGetRowsRequest) string {
	rowGroupCols := request.RowGroupCols
	groupKeys := request.GroupKeys
...
func (r *PurchaseRepository) createOrderBySQLForGoodsReceiptNoteItems(request query.IServerSideGetRowsRequest) string {
...
func (r *PurchaseRepository) createLimitAndOffsetSQLForGoodsReceiptNoteItems(request query.IServerSideGetRowsRequest) (limit int, offset int) {
