...
func (r *PurchaseRepository) createSelectSQLForRequestForQuotations(request query.IServerSideGetRowsRequest) string {
    ...
    if len(rowGroupCols) > len(groupKeys) {
        ...
        for i := 0; i <= len(groupKeys) && i < len(rowGroupCols); i++ {
            colsToSelect = append(colsToSelect, r.handleNestedFieldsForRequestForQuotations(rowGroupCols[i].Field))
        }

        // Include aggregated columns in SELECT
        for _, valueCol := range valueCols {
            colsToSelect = append(colsToSelect, fmt.Sprintf("SUM(%s) AS %s", r.handleNestedFieldsForRequestForQuotations(valueCol.Field), valueCol.Field))
        }
    } else {
        // If not grouping, select all columns directly
        for _, valueCol := range valueCols {
            colsToSelect = append(colsToSelect, r.handleNestedFieldsForRequestForQuotations(valueCol.Field))
        }
    }
...
func (r *PurchaseRepository) createWhereSQLForRequestForQuotations(request query.IServerSideGetRowsRequest) string {
    ...
    for i, groupKey := range request.GroupKeys {
        ...
        whereParts = append(whereParts, fmt.Sprintf("%s = '%s'", r.handleNestedFieldsForRequestForQuotations(colName), groupKey))
    }
...
func (r *PurchaseRepository) handleNestedFieldsForRequestForQuotations(field string) string {
...
func (r *PurchaseRepository) createFilterSQLForRequestForQuotations(colID string, filterItem query.FilterItem) string {
	...
	case "date":
		return r.createDateFilterSQLForRequestForQuotations(colID, filterItem)
	default:
		return "true"
	}
}
	...
	if filterItem.Operator != "" {
		...
		case "AND":
			...
			for _, condition := range conditions {
				andParts = append(andParts, r.createNumberFilterSQLForRequestForQuotations(colID, condition))
			}
			return strings.Join(andParts, " AND ")
		case "OR":
			var orParts []string
			for _, condition := range conditions {
				orParts = append(orParts, r.createNumberFilterSQLForRequestForQuotations(colID, condition))
			}
...
func (r *PurchaseRepository) createTextFilterSQLForRequestForQuotations(colID string, filterModel query.FilterItem) string {
	...
	if filterModel.Operator != "" {
		...
		case "AND":
			var andParts []string
			for _, condition := range conditions {
				andParts = append(andParts, r.createTextFilterSQLForRequestForQuotations(colID, condition))
			}
			return strings.Join(andParts, " AND ")
		case "OR":
			var orParts []string
			for _, condition := range conditions {
				orParts = append(orParts, r.createTextFilterSQLForRequestForQuotations(colID, condition))
			}
			return strings.Join(orParts, " OR ")
		default:
			return "false"
...
func (r *PurchaseRepository) createDateFilterSQLForRequestForQuotations(colID string, filterModel query.FilterItem) string {
	// Handle filter operator and conditions (recursive)
	if filterModel.Operator != "" {
		conditions := filterModel.Conditions
		switch filterModel.Operator {
		case "AND":
			var andParts []string
			for _, condition := range conditions {
				andParts = append(andParts, r.createDateFilterSQLForRequestForQuotations(colID, condition))
			}
			return strings.Join(andParts, " AND ")
		case "OR":
			var orParts []string
			for _, condition := range conditions {
				orParts = append(orParts, r.createDateFilterSQLForRequestForQuotations(colID, condition))
			}
			return strings.Join(orParts, " OR ")
		default:
			return "false"
