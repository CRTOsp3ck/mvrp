...
func (r *InvoiceRepository) SearchPaymentReceiptViews(ctx context.Context, exec boil.ContextExecutor, dto dto.SearchPaymentReceiptDTO) (invoice.PaymentReceiptViewSlice, int, error) {
	return r.BuildSearchQueryForPaymentReceiptViews(ctx, exec, dto)
	...
	/*
		...
		return invoice.PaymentReceiptViews(queryMods...).All(ctx, exec)
	*/
}
...
func (r *InvoiceRepository) BuildSearchQueryForPaymentReceiptViews(ctx context.Context, exec boil.ContextExecutor, dto dto.SearchPaymentReceiptDTO) (invoice.PaymentReceiptViewSlice, int, error) {
	var queryMods []qm.QueryMod

	request := dto.IServerSideGetRowsRequest

	selectSQL := r.createSelectSQLForPaymentReceiptViews(request)
	if selectSQL != "" {
		queryMods = append(queryMods, qm.Select(selectSQL))
	}

	whereSQL := r.createWhereSQLForPaymentReceiptViews(request)
	if whereSQL != "" {
		queryMods = append(queryMods, qm.Where(whereSQL))
	}

	groupBySQL := r.createGroupBySQLForPaymentReceiptViews(request)
	if groupBySQL != "" {
		queryMods = append(queryMods, qm.GroupBy(groupBySQL))
	}

	orderBySQL := r.createOrderBySQLForPaymentReceiptViews(request)
	if orderBySQL != "" {
		queryMods = append(queryMods, qm.OrderBy(orderBySQL))
	}

	limitSQL, offsetSQL := r.createLimitAndOffsetSQLForPaymentReceiptViews(request)
	if limitSQL > 0 {
	...
	count, err := invoice.PaymentReceiptViews(countQueryMods...).Count(ctx, exec)
	if err != nil {
		return nil, 0, err
	}

	res, err := invoice.PaymentReceiptViews(queryMods...).All(ctx, exec)
	return res, int(count), err
}

// 1. Create Select SQL
func (r *InvoiceRepository) createSelectSQLForPaymentReceiptViews(request query.IServerSideGetRowsRequest) string {
    ...
    if len(rowGroupCols) > len(groupKeys) {
        ...
        for i := 0; i <= len(groupKeys) && i < len(rowGroupCols); i++ {
            colsToSelect = append(colsToSelect, r.handleNestedFieldsForPaymentReceiptViews(rowGroupCols[i].Field))
        }

        // Include aggregated columns in SELECT
        for _, valueCol := range valueCols {
            colsToSelect = append(colsToSelect, fmt.Sprintf("SUM(%s) AS %s", r.handleNestedFieldsForPaymentReceiptViews(valueCol.Field), valueCol.Field))
        }
    } else {
        // If not grouping, select all columns directly
        for _, valueCol := range valueCols {
            colsToSelect = append(colsToSelect, r.handleNestedFieldsForPaymentReceiptViews(valueCol.Field))
        }
    }
...
func (r *InvoiceRepository) createWhereSQLForPaymentReceiptViews(request query.IServerSideGetRowsRequest) string {
    var whereParts []string

    // Handle group keys (if any)
    for i, groupKey := range request.GroupKeys {
        colName := request.RowGroupCols[i].Field
        whereParts = append(whereParts, fmt.Sprintf("%s = '%s'", r.handleNestedFieldsForPaymentReceiptViews(colName), groupKey))
    }

    // Handle filter model (apply filters to each column)
    for colID, filterItem := range request.FilterModel {
        whereParts = append(whereParts, r.createFilterSQLForPaymentReceiptViews(colID, filterItem))
    }
...
func (r *InvoiceRepository) handleNestedFieldsForPaymentReceiptViews(field string) string {
...
func (r *InvoiceRepository) createFilterSQLForPaymentReceiptViews(colID string, filterItem query.FilterItem) string {
	...
	case "text":
		return r.createTextFilterSQLForPaymentReceiptViews(colID, filterItem)
	case "number":
		// Cast the JSONB text value to a number
		if strings.Contains(colID, "->>'") {
			colID = fmt.Sprintf("(%s)::numeric", colID)
		}
		return r.createNumberFilterSQLForPaymentReceiptViews(colID, filterItem)
	case "date":
		return r.createDateFilterSQLForPaymentReceiptViews(colID, filterItem)
	default:
		return "true"
	}
}

func (r *InvoiceRepository) createNumberFilterSQLForPaymentReceiptViews(colID string, filterItem query.FilterItem) string {
	...
	if filterItem.Operator != "" {
		...
		case "AND":
			...
			for _, condition := range conditions {
				andParts = append(andParts, r.createNumberFilterSQLForPaymentReceiptViews(colID, condition))
			}
			return strings.Join(andParts, " AND ")
		case "OR":
			var orParts []string
			for _, condition := range conditions {
				orParts = append(orParts, r.createNumberFilterSQLForPaymentReceiptViews(colID, condition))
			}
...
func (r *InvoiceRepository) createTextFilterSQLForPaymentReceiptViews(colID string, filterModel query.FilterItem) string {
	// Handle filter operator and conditions (recursive)
	if filterModel.Operator != "" {
		...
		case "AND":
			var andParts []string
			for _, condition := range conditions {
				andParts = append(andParts, r.createTextFilterSQLForPaymentReceiptViews(colID, condition))
			}
			return strings.Join(andParts, " AND ")
		case "OR":
			var orParts []string
			for _, condition := range conditions {
				orParts = append(orParts, r.createTextFilterSQLForPaymentReceiptViews(colID, condition))
			}
			return strings.Join(orParts, " OR ")
		default:
			return "false"
...
func (r *InvoiceRepository) createDateFilterSQLForPaymentReceiptViews(colID string, filterModel query.FilterItem) string {
	// Handle filter operator and conditions (recursive)
	if filterModel.Operator != "" {
		conditions := filterModel.Conditions
		switch filterModel.Operator {
		case "AND":
			var andParts []string
			for _, condition := range conditions {
				andParts = append(andParts, r.createDateFilterSQLForPaymentReceiptViews(colID, condition))
			}
			return strings.Join(andParts, " AND ")
		case "OR":
			var orParts []string
			for _, condition := range conditions {
				orParts = append(orParts, r.createDateFilterSQLForPaymentReceiptViews(colID, condition))
			}
			return strings.Join(orParts, " OR ")
		default:
			return "false"
...
func (r *InvoiceRepository) createGroupBySQLForPaymentReceiptViews(request query.IServerSideGetRowsRequest) string {
	rowGroupCols := request.RowGroupCols
	groupKeys := request.GroupKeys
...
func (r *InvoiceRepository) createOrderBySQLForPaymentReceiptViews(request query.IServerSideGetRowsRequest) string {
...
func (r *InvoiceRepository) createLimitAndOffsetSQLForPaymentReceiptViews(request query.IServerSideGetRowsRequest) (limit int, offset int) {
