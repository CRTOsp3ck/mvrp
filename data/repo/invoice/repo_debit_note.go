...
func (r *InvoiceRepository) SearchDebitNotes(ctx context.Context, exec boil.ContextExecutor, dto dto.SearchDebitNoteDTO) (invoice.DebitNoteSlice, int, error) {
	return r.BuildSearchQueryForDebitNotes(ctx, exec, dto)
...
func (r *InvoiceRepository) BuildSearchQueryForDebitNotes(ctx context.Context, exec boil.ContextExecutor, dto dto.SearchDebitNoteDTO) (invoice.DebitNoteSlice, int, error) {
	var queryMods []qm.QueryMod

	request := dto.IServerSideGetRowsRequest

	selectSQL := r.createSelectSQLForDebitNotes(request)
	if selectSQL != "" {
		queryMods = append(queryMods, qm.Select(selectSQL))
	}

	whereSQL := r.createWhereSQLForDebitNotes(request)
	if whereSQL != "" {
		queryMods = append(queryMods, qm.Where(whereSQL))
	}

	groupBySQL := r.createGroupBySQLForDebitNotes(request)
	if groupBySQL != "" {
		queryMods = append(queryMods, qm.GroupBy(groupBySQL))
	}

	orderBySQL := r.createOrderBySQLForDebitNotes(request)
	if orderBySQL != "" {
		queryMods = append(queryMods, qm.OrderBy(orderBySQL))
	}

	limitSQL, offsetSQL := r.createLimitAndOffsetSQLForDebitNotes(request)
	if limitSQL > 0 {
	...
	count, err := invoice.DebitNotes(countQueryMods...).Count(ctx, exec)
	if err != nil {
		return nil, 0, err
	}

	res, err := invoice.DebitNotes(queryMods...).All(ctx, exec)
	return res, int(count), err
}

// 1. Create Select SQL
func (r *InvoiceRepository) createSelectSQLForDebitNotes(request query.IServerSideGetRowsRequest) string {
    ...
    if len(rowGroupCols) > len(groupKeys) {
        ...
        for i := 0; i <= len(groupKeys) && i < len(rowGroupCols); i++ {
            colsToSelect = append(colsToSelect, r.handleNestedFieldsForDebitNotes(rowGroupCols[i].Field))
        }

        // Include aggregated columns in SELECT
        for _, valueCol := range valueCols {
            colsToSelect = append(colsToSelect, fmt.Sprintf("SUM(%s) AS %s", r.handleNestedFieldsForDebitNotes(valueCol.Field), valueCol.Field))
        }
    } else {
        // If not grouping, select all columns directly
        for _, valueCol := range valueCols {
            colsToSelect = append(colsToSelect, r.handleNestedFieldsForDebitNotes(valueCol.Field))
        }
    }
...
func (r *InvoiceRepository) createWhereSQLForDebitNotes(request query.IServerSideGetRowsRequest) string {
    var whereParts []string

    // Handle group keys (if any)
    for i, groupKey := range request.GroupKeys {
        colName := request.RowGroupCols[i].Field
        whereParts = append(whereParts, fmt.Sprintf("%s = '%s'", r.handleNestedFieldsForDebitNotes(colName), groupKey))
    }

    // Handle filter model (apply filters to each column)
    for colID, filterItem := range request.FilterModel {
        whereParts = append(whereParts, r.createFilterSQLForDebitNotes(colID, filterItem))
    }
...
func (r *InvoiceRepository) handleNestedFieldsForDebitNotes(field string) string {
...
func (r *InvoiceRepository) createFilterSQLForDebitNotes(colID string, filterItem query.FilterItem) string {
	...
	case "text":
		return r.createTextFilterSQLForDebitNotes(colID, filterItem)
	case "number":
		// Cast the JSONB text value to a number
		if strings.Contains(colID, "->>'") {
			colID = fmt.Sprintf("(%s)::numeric", colID)
		}
		return r.createNumberFilterSQLForDebitNotes(colID, filterItem)
	case "date":
		return r.createDateFilterSQLForDebitNotes(colID, filterItem)
	default:
		return "true"
	}
}

func (r *InvoiceRepository) createNumberFilterSQLForDebitNotes(colID string, filterItem query.FilterItem) string {
	...
	if filterItem.Operator != "" {
		...
		case "AND":
			...
			for _, condition := range conditions {
				andParts = append(andParts, r.createNumberFilterSQLForDebitNotes(colID, condition))
			}
			return strings.Join(andParts, " AND ")
		case "OR":
			var orParts []string
			for _, condition := range conditions {
				orParts = append(orParts, r.createNumberFilterSQLForDebitNotes(colID, condition))
			}
...
func (r *InvoiceRepository) createTextFilterSQLForDebitNotes(colID string, filterModel query.FilterItem) string {
	// Handle filter operator and conditions (recursive)
	if filterModel.Operator != "" {
		...
		case "AND":
			var andParts []string
			for _, condition := range conditions {
				andParts = append(andParts, r.createTextFilterSQLForDebitNotes(colID, condition))
			}
			return strings.Join(andParts, " AND ")
		case "OR":
			var orParts []string
			for _, condition := range conditions {
				orParts = append(orParts, r.createTextFilterSQLForDebitNotes(colID, condition))
			}
			return strings.Join(orParts, " OR ")
		default:
			return "false"
...
func (r *InvoiceRepository) createDateFilterSQLForDebitNotes(colID string, filterModel query.FilterItem) string {
	// Handle filter operator and conditions (recursive)
	if filterModel.Operator != "" {
		conditions := filterModel.Conditions
		switch filterModel.Operator {
		case "AND":
			var andParts []string
			for _, condition := range conditions {
				andParts = append(andParts, r.createDateFilterSQLForDebitNotes(colID, condition))
			}
			return strings.Join(andParts, " AND ")
		case "OR":
			var orParts []string
			for _, condition := range conditions {
				orParts = append(orParts, r.createDateFilterSQLForDebitNotes(colID, condition))
			}
			return strings.Join(orParts, " OR ")
		default:
			return "false"
...
func (r *InvoiceRepository) createGroupBySQLForDebitNotes(request query.IServerSideGetRowsRequest) string {
	rowGroupCols := request.RowGroupCols
	groupKeys := request.GroupKeys
...
func (r *InvoiceRepository) createOrderBySQLForDebitNotes(request query.IServerSideGetRowsRequest) string {
...
func (r *InvoiceRepository) createLimitAndOffsetSQLForDebitNotes(request query.IServerSideGetRowsRequest) (limit int, offset int) {
