...
func (r *InvoiceRepository) createFilterSQLForInvoiceItems(colID string, filterItem query.FilterItem) string {
	...
	case "date":
		return r.createDateFilterSQLForInvoiceItems(colID, filterItem)
	default:
		return "true"
	}
}
	...
	if filterItem.Operator != "" {
		...
		case "AND":
			...
			for _, condition := range conditions {
				andParts = append(andParts, r.createNumberFilterSQLForInvoiceItems(colID, condition))
			}
			return strings.Join(andParts, " AND ")
		case "OR":
			var orParts []string
			for _, condition := range conditions {
				orParts = append(orParts, r.createNumberFilterSQLForInvoiceItems(colID, condition))
			}
...
func (r *InvoiceRepository) createTextFilterSQLForInvoiceItems(colID string, filterModel query.FilterItem) string {
	...
	if filterModel.Operator != "" {
		...
		case "AND":
			var andParts []string
			for _, condition := range conditions {
				andParts = append(andParts, r.createTextFilterSQLForInvoiceItems(colID, condition))
			}
			return strings.Join(andParts, " AND ")
		case "OR":
			var orParts []string
			for _, condition := range conditions {
				orParts = append(orParts, r.createTextFilterSQLForInvoiceItems(colID, condition))
			}
			return strings.Join(orParts, " OR ")
		default:
			return "false"
...
func (r *InvoiceRepository) createDateFilterSQLForInvoiceItems(colID string, filterModel query.FilterItem) string {
	// Handle filter operator and conditions (recursive)
	if filterModel.Operator != "" {
		conditions := filterModel.Conditions
		switch filterModel.Operator {
		case "AND":
			var andParts []string
			for _, condition := range conditions {
				andParts = append(andParts, r.createDateFilterSQLForInvoiceItems(colID, condition))
			}
			return strings.Join(andParts, " AND ")
		case "OR":
			var orParts []string
			for _, condition := range conditions {
				orParts = append(orParts, r.createDateFilterSQLForInvoiceItems(colID, condition))
			}
			return strings.Join(orParts, " OR ")
		default:
			return "false"
