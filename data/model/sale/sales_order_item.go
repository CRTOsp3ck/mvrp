...
func (r *salesOrderItemR) GetGoodsReturnNoteItems() GoodsReturnNoteItemSlice {
	if r == nil {
		return nil
	}
...
func (o *SalesOrderItem) GoodsReturnNoteItems(mods ...qm.QueryMod) goodsReturnNoteItemQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"sale\".\"goods_return_note_item\".\"sales_order_item_id\"=?", o.ID),
	)
...
func (salesOrderItemL) LoadGoodsReturnNoteItems(ctx context.Context, e boil.ContextExecutor, singular bool, maybeSalesOrderItem interface{}, mods queries.Applicator) error {
	var slice []*SalesOrderItem
	var object *SalesOrderItem
	...
	query := NewQuery(
		...
		qm.WhereIn(`sale.goods_return_note_item.sales_order_item_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		...
		for _, foreign := range resultSlice {
			...
			foreign.R.SalesOrderItem = object
		}
		return nil
	}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if queries.Equal(local.ID, foreign.SalesOrderItemID) {
				local.R.GoodsReturnNoteItems = append(local.R.GoodsReturnNoteItems, foreign)
				if foreign.R == nil {
					foreign.R = &goodsReturnNoteItemR{}
				}
				foreign.R.SalesOrderItem = local
				break
			}
		}
	}
...
// of the sales_order_item, optionally inserting them as new records.
// Appends related to o.R.GoodsReturnNoteItems.
// Sets related.R.SalesOrderItem appropriately.
func (o *SalesOrderItem) AddGoodsReturnNoteItems(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*GoodsReturnNoteItem) error {
	var err error
	for _, rel := range related {
		if insert {
			queries.Assign(&rel.SalesOrderItemID, o.ID)
	...
	for _, rel := range related {
		if rel.R == nil {
			...
			rel.R.SalesOrderItem = o
		}
	}
	return nil
}

// SetGoodsReturnNoteItems removes all previously related items of the
// sales_order_item replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.SalesOrderItem's GoodsReturnNoteItems accordingly.
// Replaces o.R.GoodsReturnNoteItems with related.
// Sets related.R.SalesOrderItem's GoodsReturnNoteItems accordingly.
func (o *SalesOrderItem) SetGoodsReturnNoteItems(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*GoodsReturnNoteItem) error {
	query := "update \"sale\".\"goods_return_note_item\" set \"sales_order_item_id\" = null where \"sales_order_item_id\" = $1"
	...
	if o.R != nil {
		for _, rel := range o.R.GoodsReturnNoteItems {
			queries.SetScanner(&rel.SalesOrderItemID, nil)
			if rel.R == nil {
				continue
			}

			rel.R.SalesOrderItem = nil
		}
		o.R.GoodsReturnNoteItems = nil
	}

	return o.AddGoodsReturnNoteItems(ctx, exec, insert, related...)
}

// RemoveGoodsReturnNoteItems relationships from objects passed in.
// Removes related items from R.GoodsReturnNoteItems (uses pointer comparison, removal does not keep order)
// Sets related.R.SalesOrderItem.
func (o *SalesOrderItem) RemoveGoodsReturnNoteItems(ctx context.Context, exec boil.ContextExecutor, related ...*GoodsReturnNoteItem) error {
	if len(related) == 0 {
		return nil
	}
	...
	for _, rel := range related {
		...
		if _, err = rel.Update(ctx, exec, boil.Whitelist("sales_order_item_id")); err != nil {
			return err
		}
	}
	if o.R == nil {
	...
	for _, rel := range related {
		for i, ri := range o.R.GoodsReturnNoteItems {
			if rel != ri {
				continue
			}

			ln := len(o.R.GoodsReturnNoteItems)
			if ln > 1 && i < ln-1 {
				o.R.GoodsReturnNoteItems[i] = o.R.GoodsReturnNoteItems[ln-1]
			}
			o.R.GoodsReturnNoteItems = o.R.GoodsReturnNoteItems[:ln-1]
			break
		}
	}
